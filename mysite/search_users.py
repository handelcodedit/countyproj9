...
def search_users(query):
    ...
    try:
        ...
        cursor.execute("SELECT * FROM users WHERE fname LIKE %s OR lname LIKE %s OR email LIKE %s OR username LIKE %s OR gender LIKE %s OR dob LIKE %s OR county_id LIKE %s OR ward_id LIKE %s role LIKE %s occupation LIKE %s marital_status LIKE %s education_level LIKE %s registration_date LIKE %s", (f"%{query}%", f"%{query}%" , f"%{query}%", f"%{query}%", f"%{query}%", f"%{query}%" , f"%{query}%", f"%{query}%", f"%{query}%", f"%{query}%", f"%{query}%", f"%{query}%", f"%{query}%"))
        result = cursor.fetchall()
        return result
...
try:
    ...
    if result:
        print("<html><body><h2>Search Results</h2>")
        ...
        print("<html><body><h2>No results found.</h2></body></html>")
